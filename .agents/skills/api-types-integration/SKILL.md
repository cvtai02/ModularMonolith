...
```
If you run the AppHost, kill it after previous command.

### 2. Sync API Types

Read the git diff for only `openapi-types.ts`, No diff then stop, else update file ${WorkSpaceFolder}/src/clients/shared/api/api-types.ts